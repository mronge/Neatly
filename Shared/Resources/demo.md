...
A **native macOS knowledge base** built with SwiftUI — fast, focused, and beautifully rendered. Write in markdown, connect your thinking with [[wiki links]] and #tags, and let Neatly handle the rest.
...
Neatly isn't just a markdown editor — it's a place to think. Link your notes together, tag them for easy retrieval, and let the backlinks panel show you connections you didn't know existed.
...
> [!TIP]
> Type `[[` in the editor and Neatly will suggest matching pages from your workspace. Links that resolve to an existing page appear green; unresolved links appear orange so you can spot gaps.
...
Neatly supports the full range of markdown formatting. Write in **bold**, *italic*, ***bold italic***, or ~~strikethrough~~. Combine them inline: this sentence has **bold with *nested italic* inside** for emphasis.

Use `inline code` for short snippets, variable names like `NSTextStorage`, or file paths like `~/Desktop/notes.md`.

Links look like this: [Neatly on GitHub](https://github.com/mronge/Neatly), and autolinked URLs work too: https://apple.com.
...
Neatly supports GitHub-style emoji shortcodes: :rocket: :fire: :sparkles: :tada: :heart: :100:
...
Neatly supports GitHub/Obsidian-style callouts with `> [!TYPE]` syntax:
...
```bash
# Build Neatly from the command line
xcodegen generate
...
flowchart LR
    ...
    click G "https://github.com/mronge/Neatly" "Neatly on GitHub" _blank
```
...
![Neatly Screenshot](https://raw.githubusercontent.com/mronge/Neatly/main/website/screenshot.jpg)
...
Neatly renders GitHub-style footnotes[^1], including multiple references[^note] in the same paragraph[^1].
...
This content is hidden by default using the HTML `<details>` element. Neatly animates the open/close transition smoothly.
...
This document barely scratches the surface. Start building your own #knowledge-base — create a new file, link it with `[[double brackets]]`, and watch Neatly connect the dots for you. :rocket:
