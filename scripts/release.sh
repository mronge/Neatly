...
create_clearly_dmg() {
  ...
  create-dmg \
    ...
    --icon-size 160 \
    --text-size 14 \
    --icon "Neatly.app" 170 180 \
    --hide-extension "Neatly.app" \
    --app-drop-link 490 180 \
    --no-internet-enable \
    --format UDZO \
    "$output_path" \
    build/export/Neatly.app || true
...
echo "🔨 Building Neatly v$VERSION..."
...
SPARKLE_FRAMEWORK="build/export/Neatly.app/Contents/Frameworks/Sparkle.framework"
...
codesign -f -s "$SIGNING_IDENTITY" -o runtime --timestamp \
  ...
  "build/export/Neatly.app/Contents/PlugIns/ClearlyQuickLook.appex"

# 4. Main app (outermost)
echo "  Signing Neatly.app..."
codesign -f -s "$SIGNING_IDENTITY" -o runtime --timestamp \
  --entitlements build/Clearly.entitlements \
  build/export/Neatly.app

# Verify mach-lookup entitlements survived
if ! codesign -d --entitlements :- build/export/Neatly.app 2>/dev/null | grep -q "mach-lookup"; then
  echo "❌ mach-lookup entitlements missing after re-sign. Aborting."
  exit 1
fi

# Verify iCloud entitlements survived
scripts/verify-entitlements.sh build/export/Neatly.app

# Deep signature chain verification
codesign --verify --deep --strict build/export/Neatly.app
echo "✅ Code signature verified (deep + strict)."

if $DRY_RUN; then
  echo "🏁 Dry run complete. Signed app at: build/export/Neatly.app"
  echo "   To inspect: codesign -d --entitlements :- build/export/Neatly.app"
  echo "   Note: spctl --assess will fail until notarized (expected in dry-run)."
  exit 0
fi

echo "📦 Creating DMG..."
create_clearly_dmg build/Neatly.dmg

echo "🔏 Notarizing..."
xcrun notarytool submit build/Neatly.dmg \
  --keychain-profile "AC_PASSWORD" \
  --wait

echo "📎 Stapling..."
xcrun stapler staple build/export/Neatly.app
rm build/Neatly.dmg
create_clearly_dmg build/Neatly.dmg
xcrun stapler staple build/Neatly.dmg || echo "⚠️  DMG staple failed (normal — CDN propagation delay). App inside is stapled."

# Gatekeeper assessment (must run after notarization + stapling)
spctl --assess --type execute --verbose build/export/Neatly.app
...
SIGNATURE=$("$SPARKLE_BIN/sign_update" build/Neatly.dmg 2>&1)
...
$DESC_ELEMENT
      <enclosure
        url="https://github.com/mronge/Neatly/releases/download/v$VERSION/Neatly.dmg"
...
if [ -n "$CHANGELOG_MD" ]; then
  gh release create "v$VERSION" build/Neatly.dmg \
    --title "Neatly v$VERSION" \
    --notes "$CHANGELOG_MD"
else
  gh release create "v$VERSION" build/Neatly.dmg \
    --title "Neatly v$VERSION" \
...
echo "✅ Done! Release: https://github.com/mronge/Neatly/releases/tag/v$VERSION"
