<p align="center">
  <img src="website/icon.png" width="128" height="128" alt="Neatly icon" />
</p>

<h1 align="center">Neatly</h1>
...
<p align="center">
  ...
  <a href="https://github.com/mronge/Neatly/releases/latest/download/Neatly.dmg">Direct Download</a> &middot;
  <a href="https://clearly.md">Website</a> &middot;
  <a href="https://x.com/Shpigford">@Shpigford</a>
</p>

<p align="center">
  <img src="website/screenshots/screenshot-1.jpg" width="720" alt="Neatly — editor with sidebar and document outline" />
</p>
...
```bash
git clone https://github.com/mronge/Neatly.git
cd Neatly
...
1. **Clearly** — main app target. `DocumentGroup` with `MarkdownDocument`, editor and preview modes, file explorer, vault indexing.
...
The `clearly` command-line binary is bundled with Neatly.app and operates on the same SQLite index the app maintains — no separate configuration, no data duplication.

### Install

Open Neatly → **Settings → Command Line → Install**. Creates a symlink at `~/.local/bin/clearly` pointing to the bundled binary inside `Neatly.app/Contents/Resources/Helpers/ClearlyCLI`. No admin password needed — everything stays in your home folder.
...
**Legacy `/usr/local/bin/clearly` installs** from older versions keep working and are detected automatically — no action needed.
...
- **Dev-build SIGKILL** — the bundled `ClearlyCLI` inside `Neatly Dev.app/Contents/Resources/Helpers/` gets SIGKILL'd by macOS when launched standalone (code-signature invalidation). Use the product binary at `Build/Products/Debug/ClearlyCLI` directly for local testing.
...
Settings → Command Line → **Copy MCP config** in the Neatly app copies a ready-to-paste snippet with the correct path for your machine (flips to `~/.local/bin/clearly` once the symlink is installed, or the legacy `/usr/local/bin/clearly` path if you're on an older install).
